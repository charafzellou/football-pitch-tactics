...
**Reseeding never touches a live save.** `server/db/seed.ts`'s delete/insert pass is scoped to `game_id IS NULL` throughout — deletes, the economy recalculation pass, everything. An unscoped `DELETE FROM teams` at reseed time would also wipe every save's cloned teams (and cascade through their players, matches, and finances), since template and clone rows share the same table.
