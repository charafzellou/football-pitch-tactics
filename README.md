...
The game features:
- A match simulation engine based on weighted events, team tactics, player ability, fitness, injuries, and disciplinary state.
...
- Full league matchdays: fixtures outside the player's match are simulated automatically, so the table and every club's last-five form reflect the complete competition.
- Team management with saved formations and starting XIs, transfers, contracts, finances, facilities, board expectations, and season progression.
...
- SQLite for persistent local save data and the seeded football world.
...
3. Once the team is selected, the player is taken to the main game screen, where they can manage their team, save a formation and starting XI, view match schedules, and participate in matches. The saved pre-match combination carries into the next fixture; substitutions and tactical changes made during a match remain specific to that match.
...
7. Once all the scheduled matches are played, the player can view the final league standings and receive a summary of their team's performance throughout the season. During the season, every standings row also shows up to five recent results, including AI-versus-AI fixtures.
...
- **Tactics and Team Sheet**: The saved formation persists between matchdays. The saved starting XI also carries forward, but is cleared at a season rollover because transfers and retirements may invalidate its player ids.
- **Squad Quality**: An assessment of the team's overall player quality and availability. No club may sell below 16 active players or below the positional floors of 2 goalkeepers, 5 defenders, 5 midfielders, and 3 attackers; injured players still count as squad members.
...
- **Score**: The current score of the match (Home Team - Away Team), derived from the events revealed at the current match minute. Goal celebrations use the same event-time state as the live score.
- **Match Events**: A list of key events that occur during the match (e.g., goals, yellow cards, substitutions).

For the complete functional and technical reference, see the [documentation index](docs/README.md).
